...
// check smd5 password
	...
	$without_salt = explode($salt,$hash);
	  ...
	  $_SESSION['ahy3yudeev8eez4eeshaikeirahG8theiyoi5uki'] = $createdsessionsecret ;
...
//	  if(isset($_SESSION['username']))  { // bad practice, wen don't need the username in cookies
	  if(isset($_SESSION['ahy3yudeev8eez4eeshaikeirahG8theiyoi5uki']))  { 
	// Change the file to the filename you want.
	// Look in the .htaccess file to block other file extensions from being shown.
	  $file = 'file.txt';
	  if (file_exists($file)) {
	  // test if file exists
//	  $file = =$_GET['file']; // for use the GET variable
	  ...
	  switch ($ftype) {
	  // switch filetypes !! This must be filled with the correct content for browsers to work
	  // gif
	  case '.gif':
